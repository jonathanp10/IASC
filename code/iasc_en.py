...
def get_timestamp_lst(filename):
    ...
    filename.replace("temprature_humidity_records_", "")
...
def send_file_to_gw(filename):
    #en_tx_manager.send_file_to_gw_with_lora(filename)
    send_file_to_gw_with_lora(filename)

# while True:
#   if no files: sleep (waking up by signal of new file)
#   get_pending_files_queue() - by order of critical - only the ones weren't sent
#   for open_file in queue:
#   	optional - process data/compress
#       send_file_to_gw(file)
#   	send_meta_data - id, filename
#   	send_file_data
#   	optional - send "finished"

pending_files_queue = get_pending_files_queue()
for pending_file in pending_files_queue:
    send_file_to_gw(pending_file)
print("[iasc_en]: The sorted queue is: " + str(pending_files_queue))
